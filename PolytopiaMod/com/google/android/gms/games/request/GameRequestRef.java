...
public final class GameRequestRef extends zzc implements GameRequest {
    ...
    public int getRecipientStatus(String playerId) {
        for (int i = this.zzadl; i < this.zzadl + this.zzauX; i++) {
            int zzbt = this.zzabq.zzbt(i);
            if (this.zzabq.zzd("recipient_external_player_id", i, zzbt).equals(playerId)) {
                return this.zzabq.zzc("recipient_status", i, zzbt);
            }
        }
        return -1;
    }
    ...
    public List<Player> getRecipients() {
        ...
        for (int i = 0; i < this.zzauX; i++) {
            arrayList.add(new PlayerRef(this.zzabq, this.zzadl + i, "recipient_"));
        }
        return arrayList;
    }
