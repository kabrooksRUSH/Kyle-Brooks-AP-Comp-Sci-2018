...
public class zzng extends zzj<DriveId> {
    ...
    protected boolean zzb(DataHolder dataHolder, int i, int i2) {
        for (String zzce : zzrG()) {
            if (!dataHolder.zzce(zzce)) {
                return false;
            }
        }
        return true;
    ...
    protected DriveId zzm(DataHolder dataHolder, int i, int i2) {
        long j = dataHolder.zzor().getLong("dbInstanceId");
        int i3 = DriveFolder.MIME_TYPE.equals(dataHolder.zzd(zznd.zzang.getName(), i, i2)) ? 1 : 0;
