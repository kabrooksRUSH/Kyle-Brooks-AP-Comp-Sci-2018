...
public class zzc extends zzhz implements ServiceConnection {
    ...
    private void zze(long j) {
        do {
            if (!zzf(j)) {
                zzb.m12v("Timeout waiting for pending transaction to be processed.");
            }
        } while (!this.zzCB);
    }
    ...
    protected void zzfm() {
        if (!this.zzCF.isEmpty()) {
            ...
            while (true) {
                ...
                ArrayList stringArrayList = zzj.getStringArrayList(IabHelper.RESPONSE_INAPP_ITEM_LIST);
                ArrayList stringArrayList2 = zzj.getStringArrayList(IabHelper.RESPONSE_INAPP_PURCHASE_DATA_LIST);
                ArrayList stringArrayList3 = zzj.getStringArrayList(IabHelper.RESPONSE_INAPP_SIGNATURE_LIST);
